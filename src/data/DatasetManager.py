...
class DatasetManager:
    ...
    def __init__(self,
        ...
        :param max_image_size: int, maximum image size in pixels, images are resized to (max_image_size, max_image_size)
        ...
        if max_image_size > 0:
            ...
            if any(file.endswith(image_ext) for file in os.listdir(RESIZED_PATH)):
                ...
                # Check if the first image's size is not equal to the max_image_size parameter
                if img_size != (max_image_size, max_image_size):
    ...
    def __split_dataset(self, dataset_in: FuseDataset, dataset_out: FuseDataset, split: float, total_size: int) \
        ...
        :param split: float, size of the split
    ...
    def __resize_images(max_image_size: int, num_workers: int) -> None:
        ...
        :param max_image_size: int, maximum image size in pixels (will be used for height & width)
        :param num_workers: int, number of workers for multiprocessing
        """
        # Initialize ray
        ray.init(include_dashboard=False)
...
def ray_resize_images(image_paths: List[str], max_image_size: int, annotations: pd.DataFrame,
    ...
    :param max_image_size: int, image size to resize all images to (height & width)
    ...
    # Create a new blank white image of size (max_image_size, max_image_size)
    ...
    # Paste the downsized original image in the new (max_image_size, max_image_size) image
