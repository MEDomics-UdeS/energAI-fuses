...
class GuiDatasetManager(CustomDatasetManager):
    ...
    @staticmethod
    def _resize_images(image_size: int, num_workers: int, img_path: str, annotations_csv: str) -> None:
        """
        ...
        # Get ray workers IDs
        if size < num_workers:
            ids = [ray_resize_images.remote(image_paths, GUI_RESIZED_PATH, image_size, annotations_csv, i) for i in range(size)]
        else:
            ids = [ray_resize_images.remote(image_paths, GUI_RESIZED_PATH, image_size, annotations_csv, i) for i in range(num_workers)]
        ...
        for _ in trange(size, desc='Resizing images...'):
            ...
            if nb_job_left > 0:
                ...
                ids.extend([ray_resize_images.remote(image_paths, GUI_RESIZED_PATH, image_size, annotations_csv, size - nb_job_left)])
