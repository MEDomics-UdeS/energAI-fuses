...
if __name__ == "__main__":
    train_dataset = FuseDataset(
        root=TRAIN_DATAPATH, data_file=TRAIN_DATAPATH + "annotations\\annotation_updated.csv", transforms=get_transform())
    ...
    # check how much space the model takes on the device and before
    # architecture, # of layers, # of weights, amount of mem/weight
    # theoretical size of model in memory
    ...
    for epoch in range(num_epochs):
        model.train()
        i = 0
        for imgs, annotations in tqdm(train_data_loader, desc=f'Epoch {epoch}'):
            i += 1
            try:
    ...
    torch.save(model.state_dict(), TRAIN_DATAPATH+"models/test6")
